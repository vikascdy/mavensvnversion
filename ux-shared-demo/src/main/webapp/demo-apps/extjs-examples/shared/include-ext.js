...
(function() {
    ...
    var scriptEls = document.getElementsByTagName('script'),
        ...
        theme = getQueryParam('theme') || 'edifecs',
        includeCSS = !hasOption('nocss', path),
        edifecs = (theme === 'edifecs'),
	...
	{
		document.write('<link rel="stylesheet" type="text/css" href="' + path + '/../packages/ext-theme'+ suffix +'/build/resources/ext-theme' + suffix + '-all.css"/>');
	}
    document.write('<script type="text/javascript" src="' + path + '/../ext/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (edifecs) {
        ...
        edifecsPath = (repoDevMode ? path + '/..' : path) +
            '/../packages/ext-theme-edifecs/build/ext-theme-edifecs' +
            (repoDevMode ? '-dev' : '') + '.js';

        if (repoDevMode &&  window.ActiveXObject) {
            Ext = {
                _beforereadyhandler: function() {
                    Ext.Loader.loadScript({ url: edifecsPath });
                }
            };
        } else {
            document.write('<script type="text/javascript" src="' + edifecsPath + '" defer></script>');
        }
    }
